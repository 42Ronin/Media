...
**`index.html` at the root is the front door**, generated by `tools/make_index.py`. It is
generated rather than kept by hand for the same reason the script docx is: an index that
lies about what exists is worse than none, so nothing is listed unless the file is on disk
and a missing one is greyed with the command that builds it. Add a tool → add a row there.
