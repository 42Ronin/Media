...
note("A mini-simulation, not a run of taught screens. It opens once at 12.1 and stays open to "
     "the end of the section — the search bar and the results never leave the screen, and the "
     "panel on the left is what changes. The tasks earlier in the lesson each opened and closed; "
     "this one does not, because the whole point is that she is one person and this is one "
     "attempt at finding her.")
note("Each step spirals: it needs everything the step before it needed, plus one thing more. The "
     "learner is not told which technique this is — they have all of them by now, and choosing "
     "is the skill being practised. The panel moves the story on when they reach each point and "
     "does not block a route we did not predict.")
note("No score. Hints escalate as they are asked for and the learner does not leave until they "
     "have opened her record.")
note("It reuses the same simulated Clarity and the same roster as the tasks. Nothing new is "
     "unlocked for it — every control it needs was already earned.")
...
kv_table([
    ...
    ("Spiral", "Step 1 of 4. Nothing to build on yet; this is the floor.", True),
])
...
kv_table([
    ...
    ("Spiral",
     "Step 2. Re-uses step 1 — you still are not treating an empty result as an answer — and "
     "adds asking rather than guessing. Tasks 1 and 12 taught it; nobody is reminding you here.", True),
])
...
kv_table([
    ...
    ("Spiral",
     "Step 3. Re-uses steps 1 and 2, and adds the alternate spelling. Task 7 taught the swap and "
     "task 4 taught the fragment — this step needs both at once, which neither task asked for.", True),
])
...
kv_table([
    ...
    ("Spiral",
     "Step 4, and it needs all three before it plus two more — narrowing from task 5 and reading "
     "the household from task 10. Five techniques in one step, none of them named for you. That "
     "is the top of the spiral and the reason this section exists.", True),
])
